...
export const load: PageLoad = async ({ url, params, fetch: _fetch, depends, route }) => {
	...
	if (list === 'section' && rest === 'new-releases') {
		...
		if (dataType === 'appears-on') {
			request = fetch(
				`/api/spotify/artists/${artistId}/albums?include_group=appears-on&${searchParams}`
			);
	...
	const resJSON: responseJSON = await res.json();
	const getData = () => {
